...
keyboard layouts that differ from the traditional =en-us= keymap.
...
* Concept
This package first swaps the traditional ~hjkl~ movement keys with their equivalent
in the destination layout, and then tries to correct the bugs introduced by these
changes in other parts of Spacemacs. This layer only tries to make these changes
